...
def make_ledger(reference_root: Path) -> dict[str, object]:
    ...
    document: dict[str, object] = {
        ...
        "project": "MiniQuake2",
        "ledger_state": "reference_baseline",
        ...
        "acceptance_gates": [
            ...
            {"id": "G02", "plan_point": 2, "status": "planned", "criterion": "reproducible Windows x64 debug, release, and dedicated builds"},
            {"id": "G03", "plan_point": 3, "status": "planned", "criterion": "qcommon differential fixtures pass for buffers, commands, cvars, filesystem, checksums, and lifecycle"},
            {"id": "G04", "plan_point": 4, "status": "planned", "criterion": "BSP38 and asset parsers are bounds-safe; collision/PVS/area results match 3.19"},
            {"id": "G05", "plan_point": 5, "status": "planned", "criterion": "window, input, audio, UDP/loopback, timing, and headless contracts pass"},
            {"id": "G06", "plan_point": 6, "status": "planned", "criterion": "OpenGL ref API v3 path renders required scenes with accepted visual evidence"},
            {"id": "G07", "plan_point": 7, "status": "planned", "criterion": "Protocol 34 client connects to original 3.19 server and replays reference demos"},
            {"id": "G08", "plan_point": 8, "status": "planned", "criterion": "original 3.19 client connects to MiniQuake2 listen/dedicated server across map changes"},
            {"id": "G09", "plan_point": 9, "status": "planned", "criterion": "baseq2 campaign, save/load, coop, and deathmatch acceptance matrix passes"},
            {"id": "G10", "plan_point": 10, "status": "planned", "criterion": "interop, visual, soak, performance, license, asset, and reproducibility release gates pass"},
        ],
        "files": files,
        "functions": functions,
    }
