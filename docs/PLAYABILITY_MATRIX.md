...
| P02 | Player move, look and all stock weapons | weapon/ballistics goldens plus exact view-weapon offset/angle/frame interpolation, BSP/dynamic per-vertex alias lighting with UserCmd lightlevel feedback, 3.19 planar world-alias shadows with view-weapon/translucent exclusion, persistent live right/left/center handedness, custom third-person model/skin/visible weapons, 3.19 temp-entity, layered muzzle-sound, monster-flash, trail/teleport/Steam/Widow/Nuke particle-family, moving snapshot projectile-trail/light and fixed-pool lifecycle goldens | PASS (mechanics/client protocol feedback) | paired original view-model/recoil pixel capture |
...
| P05 | Stock monster frame sequencing | attack/ranged/melee with exact attack movement and mechanical sounds; collision-bound ground/step/fly/swim movement; eight-marker lost-sight PlayerTrail; 63 pain/43 death plans covering all 1,813 movement frames; stock frame sounds, Infantry/Soldier death fire, boss explosion entry, sight/search and all reachable secondary locomotion/fidget callbacks; exact Medic resurrection; live `misc_actor`; live MZ2 and beam chains; corpse bounds and physical gibs | PASS (stock tables/callbacks/movement) | paired original full-encounter trace |
| P06 | Save during an active monster sequence | Private-Save v18 attack/reaction, held death-fire frame, boss aim/refire, shared-random, Medic owner/old-enemy, live turret sight/cooldown, lost-sight pursuit, environment/power-armor, gravity/push, water state, player item-rule/cooperative checkpoint and chat-flood fields; dynamic World references, boss persistence plus live-gib/dropped-item round trips; older readers retained | PASS | strict original native machine-layout saves remain explicitly rejected |
...
| P11 | Original Protocol-34 process interop | independent bidirectional raw peer passes; the external-process harness validates the exact 3.20 image, detects distribution-client redirection and cleans up replacement processes | PARTIAL | this Steam build redirects the classic image to the protocol-incompatible rerelease; an isolated copy is rejected by Steam DRM, so the same harness must run with an unwrapped original image on a compatible host |
| P12 | Release/package/device acceptance | current 192-program Release matrix, Debug product graph, byte-reproducible/extracted-smoked packages, RTX-5080 fullscreen restart, native default-device cinematic, deterministic controller/focus/gamma/audio gates, 100,000-frame/212,822-packet retail session soak with zero rejects or pending sounds, and the 47-map/500-frame-per-map Release FPS matrix | PARTIAL (local RC complete) | second-host GPU/audio, physical hot-unplug, attached-controller/manual latency and compatible-host original process |
| P13 | Retail cinematic, demo and intermission playback | stock `d1` attract loop; `*ntro.cin+base1`; product `--cinematic` completes `idlog.cin`; both installed DM2s complete; one shared host executes DM2/map and installed unit/end chains; `--play` consumes validated queued `gamemap` | PASS (product chain) | paired original demo timing/pixel trace on compatible host |
| P14 | Product menu, inventory and volume | stock attract loop interrupted by any input into the persistent main menu, live mode restart, versioned persistent config/key capture plus in-memory map handoff, difficulty-aware `*ntro.cin+base1` New Game, durable same/cross-map slots with metadata/screenshots, Join/Start Server and DM options, rotating 3D Player Setup preview, Reset Defaults, Go to Console, single-player pause, hardware gamma, settings/quit, inventory and mixer gain | PASS (local functional matrix) | paired original UI capture and external-device evidence belong to P12 |
...
The live locomotion boundary is no longer a direct transform: the shared
`m_move.c` port traces BSP/inline/dynamic hulls, maintains ground and water
state, touches triggers, and follows the eight-marker PlayerTrail after losing
line of sight.
...
silently collapsing to one shot; dynamic gib and dropped-item entities retain
model, physics, ownership and expiry across a level save, and an active Medic cable resumes against
...
effect and loop-sound fields. Late model/sound configstrings are delivered
reliably after sign-on and registered into the live client asset generation; a
real two-client UDP gate observes a live Bolt, its indexes and matching
configstrings in both clients before the normal seven-shot Blaster kill. Remaining evidence
is visual/differential rather than a known missing player-weapon family: paired
original view-model/recoil pixel captures. Functional view-weapon interpolation,
left-projection mirroring, center suppression and live Game-API handedness are
covered. The
