...
| P14 | Product menu, inventory and volume | stock attract loop interrupted by any input into the persistent main menu, live mode restart, persistent config/key capture, difficulty-aware `*ntro.cin+base1` New Game, durable same/cross-map slots with metadata/screenshots, Join/Start Server and DM options, rotating 3D Player Setup preview, Reset Defaults, Go to Console, single-player pause, hardware gamma, settings/quit, inventory and mixer gain | PASS (local functional matrix) | paired original UI capture and external-device evidence belong to P12 |
