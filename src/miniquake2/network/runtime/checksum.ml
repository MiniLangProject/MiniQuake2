...
function table()
  ...
  while index < len(initialized)
    values[index] = initialized[index]
    index = index + 1
  end while
  return values
end function
...
function blockSequence(data, offset, count, sequence)
  ...
  length = count
  if length > 60 then length = 60 end if
  source = table()
  tableOffset = sequence % 1020
  block = bytes(length + 4)
  if length > 0 then qbio.copyInto(block, 0, data, offset, length) end if
  block[length] = source[tableOffset]
  block[length + 1] = source[tableOffset + 1]
  block[length + 2] = source[tableOffset + 2]
  block[length + 3] = source[tableOffset + 3]
  crcValue = qcrc.CRC_Block(block, 0, len(block))
  sum = 0
  index = 0
