...
function testPlayerProjectilePainAndDeath()
  ...
  api.clientThink(client, command(0))
  api.runFrame()
  assertEqual(monster.health, 10, "second single-player Blaster damage")
  runFrames(api, 5)
  api.clientThink(client, command(cvgconstants.BUTTON_ATTACK))
  api.clientThink(client, command(0))
  api.runFrame()
  assertTrue(monster.health <= 0, "third blaster killed")
  ...
  assertTrue(damageEventCount(runtime) >= 3, "ballistics damage events")
